...
function ma_ang2, x, p
  ;x=r/rs
  u1=p[0]
  u2=p[1]
  return,[1.-u1*(1-sqrt(1-(x^2)))-u2*(1-sqrt(1-(x^2)))^2]/(1-u1/3-u2/6)/!pi
end


pro limb_darkening,infile,ld=ld,n_u1,n_u2  
close,/all
...
if keyword_defined(ld) eq 0 then ld_fit=1  else ld_fit=ld ;limb darkening coefficients: 1=quadratic law & 2=non-linear law & 3 Mandel & Agol (2002) coeffs
...
;vline,0.
    ...
    if ld_fit eq 4 then begin
      start=[1.0,1.5]
      ld=mpfitfun('ma_ang2', mu[ii], flux[ii]/flux[n_elements(mu)-1], err[ii], start)
